...
define MkuImage
	mkimage -A mips -O linux -T kernel -a 0x80060000 -C $(1) $(2) \
		-e 0x80060000 -n 'MIPS KOS Linux-$(LINUX_VERSION)' \
		-d $(3) $(4)
endef
...
kos_mtd=mtdparts=spi0.0:256k(u-boot)ro,64k(u-boot-env)ro,14528k(rootfs),1280k(kernel),192k(kos),64k(art)ro,15808k@0x50000(firmware)
...
bxi2000n2v2_mtdlayout=mtdparts=spi0.0:256k(u-boot)ro,64k(u-boot-env),1408k(kernel),12416k(rootfs),2048k(user),64k(cfg),64k(oem),64k(art)ro,13824k@0x50000(firmware)
...
bxo5000n2sv2_mtdlayout=mtdparts=spi0.0:256k(u-boot)ro,64k(u-boot-env),1408k(kernel),12416k(rootfs),2048k(user),64k(cfg),64k(oem),64k(art)ro,13824k@0x50000(firmware)
...
define Image/Build/Belkin
	...
	if [ -e "$(call sysupname,$(1),$(2))" ]; then \
		edimax_fw_header -m $(5) -v "$(shell echo -n KOS$(REVISION) | cut -c -13)" \
			-n "uImage" \
...
define Image/Build/Senao
	mkdir -p $(KDIR_TMP)/$(2)/
	touch $(KDIR_TMP)/$(2)/FWINFO-KOS-$(REVISION)-$(2)
	-$(CP) ./$(2)/* $(KDIR_TMP)/$(2)/
	dd if=$(KDIR_TMP)/vmlinux-$(2).uImage \
		of=$(KDIR_TMP)/$(2)/KOS-senao-$(2)-uImage-lzma.bin bs=64k conv=sync
	dd if=$(KDIR)/root.$(1) \
		of=$(KDIR_TMP)/$(2)/KOS-senao-$(2)-root.$(1) bs=64k conv=sync
	( \
		cd $(KDIR_TMP)/$(2)/;  \
...
define Image/Build/Netgear/buildkernel
	...
	mkimage -A mips -O linux -T filesystem -C none -M $(5) \
		...
		-n 'MIPS KOS Linux-$(LINUX_VERSION)' \
...
$(eval $(call SingleProfile,AthLzma,64k,BXO2000N2SU,bxo2000n-2s-u,BXO2000n-2S-U,ttyS0,115200,$$(bxo2000n2su_mtdlayout),KRuImage))
...
$(eval $(call SingleProfile,AthLzma,64k,BXI2000N2V2,bxi2000n-2-v2,BXI2000n-2-V2,ttyS0,115200,$$(bxi2000n2v2_mtdlayout),KRuImage))
$(eval $(call SingleProfile,AthLzma,64k,BXO5000N2S,bxo5000n-2s,BXO5000n-2S,ttyS0,115200,$$(bxo5000n2s_mtdlayout),RKuImage))
$(eval $(call SingleProfile,AthLzma,64k,BXO5000N2SV2,bxo5000n-2s-v2,BXO5000n-2S-V2,ttyS0,115200,$$(bxo5000n2sv2_mtdlayout),KRuImage))
...
$(eval $(call SingleProfile,AthLzma,64k,KOSQCA953XB,kos-qca953x-b,KOS-QCA953X-B,ttyS0,115200,$$(kos_mtd),RKuImage))
